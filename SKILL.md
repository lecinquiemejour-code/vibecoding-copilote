...
> « Cadrage terminé — les quatre documents sont **validés**. Le plan d'action compte **N features**, dans cet ordre. On démarre par la première : *[nom de la feature]*. **Ce GO lance la _planification_ de cette feature : je te proposerai 3 options avant d'écrire la moindre ligne de code.** On y va ? »
...
2. **PLAN.** Propose **3 options d'implémentation** conformes aux RULES, en expliquant ton raisonnement (calibré sur le profil). Tu décris le *comment* de cette feature ; tu n'écris rien encore.
