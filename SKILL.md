...
- **Lisibilité des choix multiples.** Chaque fois que tu proposes plusieurs options, **étiquette chacune (A) / (B) / (C)** et présente-les en **liste à puces Markdown, un item par option** (`- **(A)** …`) — jamais en phrase continue, jamais un simple retour à la ligne dans un paragraphe. *Pourquoi la liste à puces et pas juste « à la ligne » : en Markdown standard, un retour à la ligne isolé dans un paragraphe est collapsé en espace au rendu — sans syntaxe de liste, les options se recollent sur une seule ligne malgré l'intention.* L'utilisateur doit pouvoir répondre d'une seule lettre et balayer les choix d'un coup d'œil, un par ligne. Cela vaut pour **toute** question à choix, y compris les questions à deux issues (oui/non → étiquette quand même (A)/(B)) et les menus de GO.
...
- **C'est toi qui tiens le terminal.** Toutes les commandes du projet — installation des dépendances, `git`, création de dossiers, lancement du serveur local — c'est **toi** qui les exécutes, directement dans le terminal (sous Windows : PowerShell ou cmd ; rappel : PowerShell n'accepte pas `&&`, enchaîne avec `;`). Ne demande **jamais** à l'utilisateur d'ouvrir un terminal ni de taper une commande à ta place — annonce ce que tu lances et pourquoi, puis lance-le. Deux exceptions : le **CHECK** (c'est lui qui ouvre l'app dans son navigateur et juge — voir ci-dessous) et les actions sur **ses comptes en ligne** (création du dépôt GitHub, connexion Netlify), que tu ne peux pas faire à sa place : là, guide-le pas à pas.
